...
public class BuildSpontaneousMetaList {

    private static BuildSpontaneousMetaList instance = null;
    private MetaList metaList;
    private Tools tools = Tools.getTools();

    private BuildSpontaneousMetaList(Context mC) {
        metaList = new MetaList();
        //construire la liste complete regarder xml parser
        try {
            InputStream is = mC.getAssets().open("metamagic_halda.xml");
    ...
    public static BuildSpontaneousMetaList getInstance(Context mC) {  //pour eviter de relire le xml à chaque fois
        if (instance == null) {
            instance = new BuildSpontaneousMetaList(mC);
        }
        return instance;
    }
