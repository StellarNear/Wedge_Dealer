...
public class ResultBuilder {
    ...
    public void addResults(LinearLayout layout) {
        ...
        if (spell.getDmg_type().equalsIgnoreCase("") || calculationSpell.nDice(spell) == 0) {
            ...
            if (spell.getDmg_type().equalsIgnoreCase("heal")) {
                ((TextView) resultTemplate.findViewById(R.id.damage_title)).setText("Soins :");
            }
            ((TextView) resultTemplate.findViewById(R.id.highscore)).setText("(record:" + spell.getHighscore(mC) + ")");
            resultTemplate.findViewById(R.id.highscore).setVisibility(View.VISIBLE);
            int sumDmg = diceList.getSum();
            if (calculationSpell.getFlatDmg(spell) > 0) {
                sumDmg += calculationSpell.getFlatDmg(spell);
            }
            if (spell.isCrit()) {
